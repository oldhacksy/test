...
public class DepositConfirmationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_deposit_confirmation);
        ButterKnife.bind(this);
        adminUser = (User)getIntent().getSerializableExtra("adminUser");
        user = (User)getIntent().getSerializableExtra("user");
        monto = (Integer) getIntent().getIntExtra("amount", 0);
        textView3.setText("Se ha depositado "+ monto+ " al usuario "+ user.getFirstName() + " " +user.getLastNameP() +  " con cuenta "+user.getDni() +" \\n "+
