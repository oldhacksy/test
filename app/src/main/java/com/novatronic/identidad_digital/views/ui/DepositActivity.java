...
public class DepositActivity extends AppCompatActivity {
    ...
    public void depositar(View view){
        ...
        if (TextUtils.isEmpty(monto) || Integer.parseInt(monto) < 1) {
            edtAmount.setError("El monto a abonar debe ser mayor a 0");
        }
    ...
    private void tryDeposit(User user, String monto) {
        ...
        try {
           ...
           user.setAmount( user.getAmount()+ Integer.parseInt(monto));
            dao.update(user);
            Intent intent = new Intent(DepositActivity.this, DepositConfirmationActivity.class);
            intent.putExtra("user", user);
            intent.putExtra("amount", Integer.parseInt(monto));
            intent.putExtra("adminUser", adminUser);
            startActivity(intent);
        } catch (SQLException e) {
            Snackbar.make(findViewById(android.R.id.content),"Hubo problemas al realizar el deposito", Snackbar.LENGTH_SHORT).show();
        }
    }
